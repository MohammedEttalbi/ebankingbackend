...
public class EbankingBackendApplication {
	...
	CommandLineRunner commandLineRunner(BankAccountService bankAccountService) {
		return args -> {
				Stream.of("Ali","kalid","Mohammed").forEach(name -> {
				...
				bankAccountService.listCustomers().forEach(customer -> {
                    try {
                        bankAccountService.saveCurrentBankAccount(Math.random()*10000,10000,customer.getId());
                    	bankAccountService.saveSavingBankAccount(Math.random()*50000,0,customer.getId());
	...
	, AccountOperationRepository accountOperationRepository) {
		return args -> {
			Stream.of("Hassan","Omar","Aicha").forEach(name -> {
			...
			customerRepository.findAll().forEach(customer -> {
				...
				savingAccount.setInterestRate(0);
